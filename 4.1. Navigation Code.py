...
class GapBarrier:
    def __init__(self):
        ...
        self.ls_str=int(round(self.scan_beams*self.right_beam_angle/(2*math.pi))) #start
        self.ls_end=int(round(self.scan_beams*self.left_beam_angle/(2*math.pi))) #end
        ...
        for i in range (self.ls_len_mod):
            ...
            if  j==1 and (proc_ranges[i,0]==0 or i==self.ls_len_mod-1):
                ...
                if  range_sum_new > range_sum: 
                        end_indx2= end_indx
                        str_indx2= str_indx 
                        range_sum = range_sum_new #reset max gap sum
	...
	den = 0
		...
		#proc_ranges[i,0] accesses distance, proc_ranges[i,1] accesses angle 
		num += proc_ranges[i,0]*proc_ranges[i,1]
		den += proc_ranges[i,0]

	best_heading = num/den
